...
// Добавление документов. Добавленный документ должен отыскаться  по поисковому запросу, который содержит слова из документа. 
...
// Поддержка минус-слов. Документы, содержащие минус-слова из поискового запроса, не включаются в результаты поиска. 
...
// Соответствие документа поисковому запросу. 
void TestMatchedDocuments() {
    ...
    {
        const auto [matched_words, status] = server.MatchDocument("dog and -cat"s, 100);
        const vector<string> expected_result = {}; // empty result, there is a minus word
        ASSERT_EQUAL(expected_result, matched_words);
...
// Поиск документа, имеющие заданный статус. 
...
//Нахождение релевантности найденных документов. 
...
void TestSearchServer() {
    ...
    RUN_TEST(TestFindAddedDocumentByDocumentWord);     // Не забудьте вызывать остальные тесты здесь
