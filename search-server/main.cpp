...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query, Predicate doc_pred) const {
        const Query query = ParseQuery(raw_query);
        auto matched_documents = FindAllDocuments(query, doc_pred);
...
private:
    ...
    template <typename Predicate>
    vector<Document> FindAllDocuments(const Query& query, Predicate doc_pred) const {
        ...
        for (const string& word : query.plus_words) {
            ...
            for (const auto& [document_id, term_freq] : word_to_document_freqs_.at(word)) {
                const auto& doc_id = documents_.at(document_id);
                if (doc_pred(document_id, doc_id.status, doc_id.rating)) {
