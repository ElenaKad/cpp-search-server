...
set<string> NonEmptyStrings(const StringContainer& strings) {
...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words) {
        if (all_of(stop_words.begin(), stop_words.end(), IsValidWord)) {
            stop_words_ = NonEmptyStrings(stop_words);
        }
        else {
...
private:
    ...
    QueryWord ParseQueryWord(string text) const {
        ...
        if (text.empty() || text[0] == '-' || !IsValidWord(text)) {
            throw invalid_argument("наличие более чем одного минуса перед словом"s);
        }
...
int main() {
    ...
    AddDocument(search_server, -1, "пушистый пёс и модный ошейник"s, DocumentStatus::ACTUAL, { 1, 2 });
    AddDocument(search_server, 3, "большой пёс скворец евгений"s, DocumentStatus::ACTUAL, { 1, 3, 2 });
